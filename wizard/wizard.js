...
function updateWizardTimeline() {
    ...
    Object.keys(wizardTimelineState.steps).forEach(stepNum => {
        ...
        if (!isAccessible) {
            ...
            // Current step - ALWAYS show as ongoing (red) until we move to next step
            // Even if completed, it stays ongoing until we advance
            ...
            if (isCompleted) {
                ...
                // Partial credit for current step if partially filled
            ...
            // Future step - ALWAYS lock until current step is completed
            timelineStep.classList.add('locked');
            step.status = 'locked';
        } else {
    ...
    // Update progress bar fill - stop just before the current step's marker
    // Progress should go from start to just before the current step's diamond
    // For step 1: 0%, step 2: ~14.3% (1/7), step 3: ~28.6% (2/7), etc.
    const progressFill = document.getElementById('wizard-timeline-progress-fill');
    if (progressFill) {
        // Calculate position: (currentStep - 1) / totalSteps
        // This positions the progress bar just before the current step's marker
...
function initializeWizardTimeline() {
    document.querySelectorAll('.timeline-step').forEach(step => {
        step.addEventListener('click', () => {
            ...
            // Prevent clicking on locked steps
...
function updateStep(step) {
    ...
    // Only allow moving to the next step if current step is completed
    // Or allow going back to previous steps
    if (step > currentStep && step === currentStep + 1) {
        // Moving to next step - check if current step is completed
    ...
    // Update progress bar - stop just before the current step's marker
    ...
    // Also update timeline progress bar
    const timelineProgressFill = document.getElementById('wizard-timeline-progress-fill');
